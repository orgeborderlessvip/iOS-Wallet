...
#import "NSObject+dicFromModel.h"
#import <objc/runtime.h>

@implementation NSObject (dicFromModel)
...
- (NSDictionary *)dictionaryFromModelRemoveNilValue {
    unsigned int count = 0;
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (key && value) {
            if ([value isKindOfClass:[NSString class]]
                ...
                // 如果model里有其他自定义模型，则递归将其转换xr为字典
                [dict setObject:[value dictionaryFromModelRemoveNilValue] forKey:key];
            }
        }
    }
