...
- (void)getUrl {
    [WDNetworkManager createWithBaseUrl:@"https://tap.borderless.vip:39685/config/static-nodes.json" WithParam:@{} withMethod:GET success:^(id result) {
        self.urlArray = result;
        
        [WDWalletManager setConnectUrl:self.urlArray.firstObject];
    } failure:^(NSError *erro) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self getUrl];
        });
    } showHUD:YES showText:@"请求地址中"];
}
...
- (void)createWithBaseUrl:(NSString *)baseurl request:(HTTP_COMMAND_LIST)taskID WithParam:(NSDictionary *)param withMethod:(HTTPRequestMethod)method success:(void(^)(id result))success failure:(void(^)(NSError *erro))failure showHUD:(BOOL)showHud showText:(NSString *)showText {
    DLog(@"%@:%@",baseurl,[self convertToJsonData:param]);
    
    if (![baseurl isEqualToString:URL_BASE]) {
...
- (void)checkUpdateWithSuccess:(void(^)(id result))success error:(void(^)())errors {
    ...
    NSString * url = @"https://tap.borderless.vip:39685/update.json";
    /******************************************************************/
