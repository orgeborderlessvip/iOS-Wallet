...
- (void)setModel:(WDBuy_SellTableDataModel *)model {
    if (model.isSell) {
        ...
        _typeLabel.text = [NSString stringWithFormat:@"%@%ld",WDLocalizedString(@"卖出"),model.count];
    }else {
        _typeLabel.text = [NSString stringWithFormat:@"%@%ld",WDLocalizedString(@"买入"),model.count];
