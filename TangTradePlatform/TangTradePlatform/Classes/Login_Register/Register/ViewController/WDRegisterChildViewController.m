...
- (void)viewDidLoad {
    ...
    [self.viewModel.registerSubject subscribeNext:^(id x) {
        @strongify(self);
        BOOL enable = [self.viewModel.password availableTest];
        
        if ([self.viewModel.password isEqualToString:self.viewModel.confirmPassword] && enable) {
            ...
            NSString *message = enable?@"密码和确认密码不一致，请修改":@"请输入数字字母组合密码";
            NSString *title = @"提示";
            NSString *confirm = @"确定";
            [UIAlertController showAlert:YES fromVC:self withTitle:title message:message withButtonTitle:@[confirm] clickAction:^(NSInteger index, NSString *title) {
