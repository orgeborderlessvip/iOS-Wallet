...
- (void)languageSet {
    ...
    _passwordTitleLabel.text = WDLocalizedString(@"密码:");
...
- (void)viewDidLoad {
    ...
    [[self.confirmButton rac_signalForControlEvents:(UIControlEventTouchUpInside)] subscribeNext:^(id x) {
        @strongify(self);
        if (self.fileName) {
//            [WDBaseFileControl addDataToLocalWithBakUpName:self.fileName andPassword:self.passwordTextField.text withSuccess:^(BOOL result) {
//                
//            }];
        }
